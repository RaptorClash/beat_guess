...
class GameController extends ChangeNotifier {
  ...
  int get songsLeft => unplayedSongs.length + (currentGuessSong != null ? 1 : 0);
  ...
  Future<void> initGame(VoidCallback onPlaylistLoadError) async {
    ...
    if (playlistUrl.isNotEmpty) {
      List<Song> fetchedSongs = await _playlistService.fetchSpotifyPlaylist(playlistUrl);
  ...
  int getNextPlayerIndex() {
    ...
    if (playUntilAllFinish) {
      int safetyCounter = 0;
      while (players[nextIndex].score >= cardsToWin && safetyCounter < players.length) {
  ...
  bool guessPlacement(int index) {
    ...
    if (index < p.timeline.length && p.timeline[index].year < songYear) isCorrect = false;
